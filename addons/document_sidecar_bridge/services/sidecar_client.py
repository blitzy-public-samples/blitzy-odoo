...
class SidecarClient:
    ...
    def render_report(self, report_type, record, report):
        ...
        if not config['enabled']:
            raise SidecarUnavailable("Sidecar service is disabled in configuration")
        
        # Validate configuration
        if not config['api_key']:
            raise SidecarUnavailable("Sidecar API key not configured")
        if not config['secret_key']:
            raise SidecarUnavailable("Sidecar secret key not configured")
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                raise SidecarUnavailable(
                    f"Sidecar returned HTTP {response.status_code}: {error_detail}"
                )
            ...
            if not pdf_base64:
                raise SidecarUnavailable("Sidecar response missing pdf_base64 field")
            ...
            _logger.warning(
                ...
                request_id, config['timeout'], str(e)
            )
