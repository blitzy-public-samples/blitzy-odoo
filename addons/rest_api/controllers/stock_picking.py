"""REST CRUD controller for the ``stock.picking`` pilot model.

This module exposes full Create/Read/Update/Delete semantics for
``stock.picking`` under the versioned ``/api/v1/stock-pickings`` path of the
...
``addons/rest_api/controllers/res_partner.py``: the five endpoints, the four
private helpers, the validation flow, the verb -> ORM mapping, the serialization
and the constraints are identical -- only the target model (``stock.picking``),
the base path (``/api/v1/stock-pickings``) and the pydantic DTOs
(``StockPicking*``) differ. The addon scope deliberately forbids a shared base
controller module, so each sibling controller keeps its own copy of these
helpers.
...
* **Validation gate (Gate 2).** Each write verb validates the *raw* request
  ...
  **before** any ``request.env['stock.picking']`` access, so a schema-invalid
  payload yields ``422`` with zero ORM side effects. Because ``stock.picking``
  carries datetime fields (``scheduled_date``, ``date_deadline``,
  ``date_done``), validating the **raw JSON body** via
  :meth:`pydantic.BaseModel.model_validate_json` is load-bearing: under the
  DTO's ``strict=True`` configuration an ISO-8601 datetime *string* validates
  only through ``model_validate_json`` (which parses the raw body), never
  through ``model_validate`` on a pre-decoded ``dict`` (which would reject it
  with a ``datetime_type`` error).
* **Authorization parity (Gate 3).** Every data operation is executed through
  ``request.env['stock.picking'].<op>()`` bound to the authenticated user -- no
  ...
  (:func:`StockPickingController._read_fields`), keeping REST reads
...
====== ======================================== ============== ============ =========
GET    ``/api/v1/stock-pickings``               ``search_read``read-only    ``read``
GET    ``/api/v1/stock-pickings/{record_id}``   ``read``       read-only    ``read``
POST   ``/api/v1/stock-pickings``               ``create``     read-write   ``create``
PATCH  ``/api/v1/stock-pickings/{record_id}``   ``write``      read-write   ``write``
DELETE ``/api/v1/stock-pickings/{record_id}``   ``unlink``     read-write   ``unlink``
====== ======================================== ============== ============ =========
...
from odoo.addons.rest_api.schemas.stock_picking import (
    StockPickingCreate,
    StockPickingList,
    StockPickingRead,
    StockPickingUpdate,
...
class StockPickingController(Controller):
    """Thin REST controller exposing ``/api/v1/stock-pickings`` CRUD endpoints.
    ...
    # ------------------------------------------------------------------
    # Collection endpoint -- GET /api/v1/stock-pickings  (ORM: search_read)
    # ------------------------------------------------------------------
    @route(
    ...
    def list_stock_pickings(self, **kwargs):
        """List stock transfers (paginated collection).
        ...
        and returns a :class:`StockPickingList` payload with pagination
        metadata.

        :return: JSON-native ``dict`` shaped as :class:`StockPickingList`
    ...
    # ------------------------------------------------------------------
    # Create endpoint -- POST /api/v1/stock-pickings  (ORM: create)
    # ------------------------------------------------------------------
    @route(
        '/api/v1/stock-pickings',
        type='rest',
    ...
    def create_stock_picking(self, **kwargs):
        ...
        The raw request body is validated against :class:`StockPickingCreate`
        **before** any ORM access. Under the DTO's ``strict=True`` /
        ...
        rendered as HTTP ``422`` -- with the ``stock.picking`` table left
        untouched (validation Gate 2).

        The raw JSON body (not a pre-parsed ``dict``) is validated so that
        ISO-8601 datetime strings for ``scheduled_date`` / ``date_deadline``
        parse correctly under ``strict=True`` (see the module docstring); a
        malformed datetime value therefore also yields ``422`` before the ORM
        is touched.
        ...
        :return: the created record serialized as :class:`StockPickingRead`.
        """
        # Validate the RAW JSON body (not a pre-parsed dict): under strict mode
        # ``model_validate_json`` correctly parses scalar/datetime literals and
        # sees exactly the client-sent keys for ``extra='forbid'`` enforcement.
        dto = StockPickingCreate.model_validate_json(
            request.httprequest.get_data(as_text=True),
        )
        model = request.env['stock.picking']
        # Translate any x2many list-of-id payloads (e.g. ``move_ids`` /
        # ``move_line_ids``) into explicit Odoo commands before create -- a bare
        # id list cannot express full-list replacement/clear semantics (see
    ...
    # ------------------------------------------------------------------
    # Single-record read -- GET /api/v1/stock-pickings/<id>  (ORM: read)
    # ------------------------------------------------------------------
    @route(
        '/api/v1/stock-pickings/<int:record_id>',
        type='rest',
        auth='rest_bearer',
        methods=['GET'],
        readonly=True,
    )
    def get_stock_picking(self, record_id, **kwargs):
        """Read a single stock transfer by id.

        :param int record_id: the ``stock.picking`` database id (an
        ...
        :return: the record serialized as :class:`StockPickingRead`.
        """
        record = request.env['stock.picking'].browse(record_id)
        ...
        if not record.exists():
            raise werkzeug.exceptions.NotFound(
                f"stock.picking {record_id} not found",
            )
        return self._serialize_record(record)

    # ------------------------------------------------------------------
    # Partial update -- PATCH /api/v1/stock-pickings/<id>  (ORM: write)
    # ------------------------------------------------------------------
    @route(
        '/api/v1/stock-pickings/<int:record_id>',
        type='rest',
    ...
    def update_stock_picking(self, record_id, **kwargs):
        """Partially update a stock transfer.

        The raw body is validated against :class:`StockPickingUpdate` (every
        field optional, still strict and ``extra``-forbidding) **before** the
        record is fetched or written, so an invalid payload -- including a
        malformed ``scheduled_date`` / ``date_deadline`` datetime -- yields
        ``422`` before any ORM access (Gate 2). Only the supplied fields are
        written (``exclude_unset=True``), giving partial-update semantics.

        :param int record_id: the ``stock.picking`` database id.
        :raises werkzeug.exceptions.NotFound: if the record does not exist or is
            not visible to the current user (HTTP ``404``).
        :return: the updated record serialized as :class:`StockPickingRead`.
        """
        # Validate FIRST (before touching request.env) so a schema-invalid body
        # 422s with zero ORM access. The raw JSON body is validated so ISO-8601
        # datetime strings parse under strict mode.
        dto = StockPickingUpdate.model_validate_json(
            request.httprequest.get_data(as_text=True),
        )
        record = request.env['stock.picking'].browse(record_id)
        if not record.exists():
        ...
        # PATCH *replaces* the full relation (and ``move_ids=[]`` /
        # ``move_line_ids=[]`` clears it) rather than leaving existing lines
        # untouched (see :meth:`_to_orm_write_values`).
        values = self._to_orm_write_values(
    ...
    # ------------------------------------------------------------------
    # Delete -- DELETE /api/v1/stock-pickings/<id>  (ORM: unlink)
    # ------------------------------------------------------------------
    @route(
        '/api/v1/stock-pickings/<int:record_id>',
        type='rest',
    ...
    def delete_stock_picking(self, record_id, **kwargs):
    ...
    def _read_fields():
        ...
        The field set is taken from ``StockPickingRead.model_fields`` minus
        ...
        :return: ordered list of ``stock.picking`` field names to read.
        :rtype: list[str]
        """
        return [name for name in StockPickingRead.model_fields if name != 'id']
    ...
    def _to_orm_write_values(model, values):
        ...
        ``move_ids`` and ``move_line_ids``) as a plain list of ids
        (``list[int]``) -- the natural REST contract, where a client always
        transmits the *complete* desired set of related ids. Odoo's ``create`` /
        ``write``, however, expect x2many values to be expressed as *commands*:
        a bare id list neither expresses a deterministic full-list replacement
        nor -- crucially -- can an empty list clear the relation (an empty
        ``list`` is a no-op on ``write``, leaving the existing lines untouched).
        ...
        than hard-coding ``move_ids`` / ``move_line_ids``) so it stays identical
        to the canonical
    ...
    def _serialize_row(model, row):
        ...
        * ``one2many`` / ``many2many`` -> a list of ids. Normalized to ``[]``
          when empty.
        * empty scalar (``Char`` / ``Selection`` / ``Datetime`` / ...) ->
          ``False``. Normalized to ``None`` -- but **only** for non-boolean
          fields, so a legitimate boolean ``False`` is preserved rather than
          corrupted into ``None``.

        The normalized mapping is round-tripped through
        ``StockPickingRead.model_validate(...).model_dump(mode='json')``: the
        DTO is lenient (``from_attributes=True``, not strict) so it tolerates
        the coerced values and drops any stray keys, and ``mode='json'`` yields
        a fully JSON-serializable dict (datetimes rendered as ISO strings).

        :param model: a ``stock.picking`` recordset (used only for field
            metadata via ``model._fields``).
        :param dict row: a single ``read`` / ``search_read`` result row.
        :return: a JSON-native dict matching the :class:`StockPickingRead` shape.
        :rtype: dict
        """
        out = {'id': row['id']}
        for fname in StockPickingController._read_fields():
            ...
            if field.type == 'many2one':
                ...
                # empty Char / Selection / Datetime / relation  ->  None
                out[fname] = None
            else:
                # genuine bool / int / float / str / datetime kept verbatim
                out[fname] = value
        return StockPickingRead.model_validate(out).model_dump(mode='json')

    @classmethod
    def _serialize_record(cls, record):
        """Serialize a single ``stock.picking`` browse record.
        ...
        :param record: a singleton ``stock.picking`` recordset.
        :return: a JSON-native dict matching the :class:`StockPickingRead` shape.
        :rtype: dict
        """
